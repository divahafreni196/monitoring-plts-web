import { getEspStatus, formatDateTime } from '../utils';
...
export default function Sidebar({ section, setSection, realtimeData, open, onClose, isAdmin, onLoginClick, onLogout }) {
  const status = getEspStatus(realtimeData?.received_at);
  const lastUpdate = formatDateTime(realtimeData?.received_at);
        ...
        <nav className="sidebar-nav">
          {NAV.map(item => (
            <a key={item.id} href="#"
              ...
              onClick={(e) => { e.preventDefault(); setSection(item.id); onClose(); }}
            >
